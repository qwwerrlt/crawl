'use strict';

const util = require('./common/util');
const fs = require('fs');
const async = require('async');

gubenjiegou();

function gubenjiegou(){
	
	async.waterfall([
        (_cb) => {
            sign(_cb);
        },
        (token, _cb) => {
			console.log(token);
            getStockList(token, _cb);
        },
        (token, list, _cb) => {
			//console.log(list);
            getContent(token, list, _cb);
        }
    ], (err) => {
        if (err) {
            console.log(err);
        }
        else {
            console.log('gubenjiegou success');
        }
    });
}

function sign(cb){
	var url = 'https://gw.yundzh.com/token/access?appid=dcdc435cc4aa11e587bf0242ac1101de&secret_key=InsQbm2rXG5z';
	util.getData(url, (err, data) => {
		if (err){
			return cb(err);
		}
		if (typeof data == 'string') {
			data = JSON.parse(data);
		}
		cb(null, data.Data.RepDataToken[0].token);
	})
}

function getStockList(token, cb) {

	var link = 'https://gw.yundzh.com/stkdata?gql=block=' +
		encodeURIComponent('股票') + '\\' + encodeURIComponent('市场分类') + '\\'
		+ encodeURIComponent('全部A股') + '&token=' + token;

	util.getData(link,(err, data) => {
		if (err) {
			return cb(err);
		} else if (!data) {
			return cb('no data');
		} else if (data.Err) {
			return cb(data.Err);
		} else if (typeof data == 'string') {
			try {
				data = JSON.parse(data);
			} catch (e) {
				return cb(e)

			}

		}
		
		if (!data.Data || !data.Data.RepDataStkData){
			console.log(JSON.stringify(data));
			return cb('data exception');
		}
		
		let list = [];
		let stocks = data.Data.RepDataStkData;
		stocks.forEach((item) => {
			list.push(item.Obj);
		});
		cb(null, token, list);
	});
}

function getContent(token, list, cb){
	
	async.eachSeries(list, (stock, _cb) => {
		var myData = {
			stock: stock,
		}
		async.waterfall([
			(__cb) => {
				let url = "https://gw.yundzh.com/f10/gbfh/gbjg?obj=";
				let link = url + stock + '&token=' + token;
				getGubenjiegou(link, __cb);
			},
			(guben, __cb) => {
				myData.gubenjiegou = guben;
				let url = "https://gw.yundzh.com/f10/dstx/jjlt?obj=";
				let link = url + stock + '&token=' + token;
				getXianshoujiejin(link, __cb);
			},
			(xianshoujiejin, __cb) => {
				myData.xianshoujiejin = xianshoujiejin;
				let url = "https://gw.yundzh.com/f10/gdjc/gdhs?obj=";
				let link = url + stock + '&token=' + token;
				getGudongmingxi(link, __cb);
			},
			(gudongmingxi, __cb) => {
				myData.gudongmingxi = gudongmingxi;
				let outputFilename = '../f10/gubenjiegou/' + stock + '.json';
				fs.writeFile(outputFilename, JSON.stringify(myData, null, 4), __cb);
			}], (err) => {
				if (err) return _cb(err);
				_cb();
			}
		);
	},(err) => {
		if (err) return cb(err);
		cb();
	})
	
}


function getGubenjiegou(url, cb) {
	
	util.getData(url, (err, data) => {
			
		if (err) {
			return cb(err);
		} else if (!data) {
			return cb('no data');
		} else if (data.Err) {
			return cb(data.Err);
		} else if (typeof data == 'string') {
			try {
				data = JSON.parse(data);
			} catch (e) {
				return cb(e)
			}
		}
			
		if (!data.Data || !data.Data.RepDataF10GbfhGbjgOutput || !data.Data.RepDataF10GbfhGbjgOutput[0]){
			console.log(url, data,'data exception');
			return cb(null, {});
		}
		
		var infos = data.Data.RepDataF10GbfhGbjgOutput[0].Data;
		if (infos.length < 1) {
			return cb('no data')
		}
		var info = infos[infos.length - 1];
		cb(null, {Zgb: info.Zgb, Ltgf: info.Ltgf, Xsltg: info.Xsltg});
	});
}

function getXianshoujiejin(url, cb) {
	
		util.getData(url, (err, data) => {
	
			if (err) {
				return cb(err);
			} else if (!data) {
				return cb('no data');
			} else if (data.Err) {
				return cb(data.Err);
			} else if (typeof data == 'string') {
				try {
					data = JSON.parse(data);
				} catch (e) {
					return cb(e)
				}
			}
				
			if (!data.Data || !data.Data.RepDataF10DstxJjltOutput || !data.Data.RepDataF10DstxJjltOutput[0]){
				console.log(url, data,'data exception');
				return cb(null, []);
			}
			
			var infos = data.Data.RepDataF10DstxJjltOutput[0].Data;
			var list = [];
			infos.forEach(function(item) {
				var info = {};
				info.date = item.date;
				info.jjgf = item.jjgf;
				info.zzgf = item.zzgf;
				list.push(info);
			})
			cb(null, list);
		});
}

function getGudongmingxi(url, cb) {
	
	util.getData(url, (err, data) => {
			
		if (err) {
			return cb(err);
		} else if (!data) {
			return cb('no data');
		} else if (data.Err) {
			return cb(data.Err);
		} else if (typeof data == 'string') {
			try {
				data = JSON.parse(data);
			} catch (e) {
				return cb(e)
			}
		}
			
		if (!data.Data || !data.Data.RepDataF10GdjcGdhsOutput || !data.Data.RepDataF10GdjcGdhsOutput[0]){
			console.log(url, data,'data exception');
			return cb(null, {});
		}
		
		var infos = data.Data.RepDataF10GdjcGdhsOutput[0].Data;
		if (!infos || infos.length < 1) {
			console.log(infos, 'special data');
			return cb(null, {});
		}
		var info = infos[infos.length - 1];
		cb(null, {date: info.date, Gdzhs: info.Gdzhs, Hbbh: info.Hbbh, Rjcg: info.Rjcg});
	});
}